...
## 19. Streaming Réel du Premier Passage de Chat
- **Problème corrigé** : `sevenAgent.chatStream()` ne streamait réellement que la « passe 2 » (la reformulation qui suit l'exécution d'un outil), via `model.generateContentStream()`. La « passe 1 » — l'appel initial, qui couvre à la fois la détection d'appel d'outil **et** la très grande majorité des cas : une réponse conversationnelle simple sans outil — attendait la réponse complète de Gemini via `generateContent()` non-streamé, puis simulait un effet de streaming en redécoupant le texte déjà entier en tranches de 6 caractères (`replayAsChunks`). L'app *donnait l'impression* de streamer sans le faire réellement à l'appel API, ce qui ajoutait de la latence perçue avant le premier mot affiché.
- **Correctif** : `src/core/sevenAgent.ts` — la passe 1 utilise maintenant `model.generateContentStream()` pour de vrai. Chaque chunk est inspecté : s'il contient un `functionCall`, le flux s'arrête immédiatement et la logique d'exécution d'outil prend le relais (un appel de fonction ne contient aucun texte à afficher progressivement) ; sinon le texte du chunk est transmis à `onToken()` dès qu'il arrive, sans attendre la fin de la réponse. `src/core/geminiClient.ts` expose désormais `candidates` sur chaque chunk streamé (pas seulement le texte concaténé) pour permettre cette détection. En cas d'échec du flux lui-même (coupure réseau en cours de streaming), un repli vers un unique appel `generateContent()` tamponné évite de perdre la réponse.
- **Aucun changement d'affichage nécessaire** : `app/chat.tsx` et `app/index.tsx` appelaient déjà `chatStream()` avec un `onToken` qui met à jour la bulle en direct, et `ChatBubble.tsx` gérait déjà l'affichage progressif — seul le fond (vrai streaming API vs. rejeu simulé) a changé.
- **Tests** : `__tests__/sevenAgent.chatStream.test.ts` (3 tests) pin le streaming token-par-token d'une réponse conversationnelle, la détection d'un `functionCall` en cours de flux + l'enchaînement outil → passe 2 streamée, et le repli tamponné si le flux lève une erreur.

---

## 20. Barge-in Vocal — Interrompre SEVEN en Parlant
- **Interaction naturelle** : en mode mains libres ou dans l'interface vocale immersive, le microphone reste à l'écoute en priorité d'arrière-plan pendant la réponse audio. Le premier résultat STT partiel qui correspond à la voix de l'utilisateur coupe immédiatement Fish Audio, ElevenLabs ou la synthèse système ; la capture continue ensuite jusqu'à la fin de la phrase et envoie l'interruption complète comme nouveau tour.
- **Une seule session microphone** : le radar de barge-in passe par l'arbitrage existant de `useVoice` (`liveSession`, propriétaire et priorité). Il ne crée jamais un deuxième client audio en parallèle et cède devant toute capture explicite de premier plan.
- **Protection contre l'écho** : `src/core/voiceBargeIn.ts` normalise accents et ponctuation, puis compare les mots reconnus au texte que SEVEN prononce. Une phrase majoritairement identique est considérée comme le haut-parleur repris par le micro, oubliée, puis l'écoute est relancée ; les commandes courtes explicites (« stop », « arrête », « attends », etc.) passent toujours. Cela évite de renvoyer la propre réponse de l'assistant comme message utilisateur sans prétendre fournir une annulation d'écho matérielle indisponible via Expo.
- **Contrôle utilisateur** : le réglage **Voice barge-in / Interruption vocale** est activé par défaut, désactivable dans la carte AUDIO DSP, et persisté avec la configuration.
- **Tests** : `__tests__/voiceBargeIn.test.ts` couvre écho français/anglais, accents, phrases distinctes et commandes d'arrêt ; `__tests__/voiceCapture.test.ts` vérifie qu'un partiel non-écho est remonté immédiatement, qu'un écho est ignoré sans fermer la session, puis que la phrase utilisateur complète est livrée.
