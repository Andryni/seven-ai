...
class WebSearchService {
  ...
  public async searchWeb(query: string, language: 'fr' | 'en' = 'en'): Promise<WebSearchResponse> {
    ...
    if (config.privacyProfile === 'local' || config.cloudTextEnabled === false) {
      throw new Error('Web search is disabled by the active privacy profile.');
    }
    if (config.cloudJournalEnabled !== false) cloudJournalService.record({ provider: config.braveSearchApiKey ? 'brave' : 'other', action: 'web search', status: 'sent', dataClass: 'search' }).catch(() => {});
    const results: SearchResultItem[] = [];
    const braveKey = config.braveSearchApiKey || '';
