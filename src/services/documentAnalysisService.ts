...
export interface DocumentAnalysisResult {
  ...
  fullLength?: number;
  message: string;
  securityFindings?: { severity: 'medium' | 'high' | 'critical'; pattern: string; excerpt: string }[];
}
...
class DocumentAnalysisService {
  ...
  public async pickAndReadDocument(): Promise<DocumentAnalysisResult> {
    try {
      ...
      const securityFindings = securityPolicyService.scanUntrustedText(snippet);
      ...
      return {
        ...
        message: securityFindings.length
          ? `Loaded “${name}”, but isolated ${securityFindings.length} possible prompt-injection pattern(s).`
          : `Successfully loaded document "${name}" (${size ? Math.round(size / 1024) : 0} KB).`,
      };
    } catch (e: any) {
      return {
        success: false,
