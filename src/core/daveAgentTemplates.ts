/**
 * Built-in website template generators used by DaveAgentService when no
 * Gemini key is configured (or as a fallback if the AI synthesis call
 * fails). Split out of `daveAgent.ts`, which had grown past 1100 lines
 * dominated by these four large multi-KB HTML/CSS/JS string templates.
 * They are pure functions of (userName, prompt) with no dependency on
 * `DaveAgentService` instance state, so extraction is a straight move.
 */

/**
 * SaaS Landing Page Template
 */
export function generateSaaSLandingPage(userName: string, prompt: string): {
  'index.html': string;
  'style.css': string;
  'script.js': string;
} {
 ...
 */
export function generateRoboticsDashboard(userName: string, prompt: string): {
  'index.html': string;
  'style.css': string;
  'script.js': string;
} {
 ...
 */
export function generateArcadeGame(userName: string, prompt: string): {
  'index.html': string;
  'style.css': string;
  'script.js': string;
} {
 ...
 */
export function generateMasterDeveloperPortfolio(userName: string, prompt: string): {
  'index.html': string;
  'style.css': string;
  'script.js': string;
} {
