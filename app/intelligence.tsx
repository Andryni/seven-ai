...
import { fetchNews, fetchWeather, type LiveNewsItem, type LiveWeather } from '../src/services/liveInfoService';
...
export default function IntelligenceScreen() {
  ...
  const refresh = useCallback(async () => {
    setRefreshing(true);
    try {
      const [nextWeather, nextNews] = await Promise.all([
...
const intelStyles = (t: Palette) => ({
  ...
  metricRow: { flexDirection: 'row', gap: 7, marginTop: 9 }, metric: { flex: 1, borderWidth: 1, borderColor: t.border, borderRadius: 7, padding: 9, backgroundColor: t.accentSoft }, metricHead: { flexDirection: 'row', alignItems: 'center', gap: 4 }, metricLabel: { color: t.textFaint, fontFamily: FONT.mono, fontSize: 6.5 }, metricValue: { color: t.text, fontFamily: FONT.display, fontSize: 13, marginTop: 7 },
