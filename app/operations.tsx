...
import { operationService, useOperations } from '../src/services/operationService';
...
export default function OperationsScreen() {
  ...
  const activeOperations = operations.filter((operation) => operation.state === 'running' || operation.state === 'queued' || operation.state === 'paused');
    ...
    <ParticleBackground>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content}>
        ...
        <View style={styles.queueHeader}>
          <Text style={styles.sectionLabel}>PERSISTENT OPERATION QUEUE</Text>
          <TapScale onPress={() => operationService.clearFinished()} style={styles.clearQueue} accessibilityLabel="Clear finished operations"><Text style={styles.clearQueueText}>PURGE FINISHED</Text></TapScale>
        </View>
        {operations.slice(0, 12).map((operation) => (
          <View key={operation.id} style={styles.operationCard}>
            <View style={styles.operationTop}>
              <View style={styles.agentBadge}><Text style={styles.agentBadgeText}>{operation.agent}</Text></View>
              <View style={styles.operationCopy}><Text style={styles.operationTitle} numberOfLines={1}>{operation.title}</Text><Text style={styles.operationDetail} numberOfLines={1}>{operation.detail || operation.kind}</Text></View>
              <Text style={[styles.operationState, operation.state === 'failed' && { color: palette.error }, operation.state === 'completed' && { color: palette.success }]}>{operation.state.toUpperCase()}</Text>
            </View>
            <View style={styles.progressTrack}><View style={[styles.progressFill, { width: `${operation.progress}%` }]} /></View>
            <View style={styles.operationSteps}>
              {operation.steps.map((step) => <View key={step.id} style={styles.operationStep}><View style={[styles.operationStepDot, step.state === 'completed' && styles.operationStepDone, step.state === 'running' && styles.operationStepRunning, step.state === 'failed' && { backgroundColor: palette.error }]} /><Text style={styles.operationStepText}>{step.label.toUpperCase()}</Text></View>)}
            </View>
            {(operation.state === 'running' || operation.state === 'queued' || operation.state === 'paused') && <View style={styles.operationActions}><TapScale style={styles.pauseOperation} onPress={() => operation.state === 'paused' ? operationService.resume(operation.id) : operationService.pause(operation.id)} accessibilityLabel={operation.state === 'paused' ? 'Resume operation' : 'Pause operation'}><Text style={styles.pauseOperationText}>{operation.state === 'paused' ? 'RESUME' : 'PAUSE'}</Text></TapScale><TapScale style={styles.cancelOperation} onPress={() => operationService.cancel(operation.id)} accessibilityLabel="Cancel operation"><Text style={styles.cancelOperationText}>CANCEL</Text></TapScale></View>}
          </View>
        ))}
        {!operations.length && <View style={styles.emptyQueue}><Text style={styles.emptyQueueText}>NO PERSISTED OPERATIONS // STANDING BY</Text></View>}
...
const operationStyles = (t: Palette) => ({
  ...
  sectionLabel: { color: t.textFaint, fontFamily: FONT.mono, fontSize: 8, letterSpacing: 1.5, marginTop: 18, marginBottom: 7 },
  queueHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  clearQueue: { marginTop: 12, borderWidth: 1, borderColor: t.border, borderRadius: 4, paddingHorizontal: 7, paddingVertical: 4 },
  clearQueueText: { color: t.textFaint, fontFamily: FONT.mono, fontSize: 6.5 },
  operationCard: { borderWidth: 1, borderColor: t.borderStrong, borderRadius: 8, backgroundColor: 'rgba(3,10,19,.94)', padding: 10, marginBottom: 7 },
  operationTop: { flexDirection: 'row', alignItems: 'center', gap: 7 },
  agentBadge: { borderWidth: 1, borderColor: t.accent, borderRadius: 4, backgroundColor: t.accentSoft, paddingHorizontal: 6, paddingVertical: 4 },
  agentBadgeText: { color: t.accent, fontFamily: FONT.display, fontSize: 7 },
  operationCopy: { flex: 1 }, operationTitle: { color: t.text, fontFamily: FONT.uiMedium, fontSize: 10, fontWeight: '800' }, operationDetail: { color: t.textFaint, fontFamily: FONT.mono, fontSize: 6.5, marginTop: 2 }, operationState: { color: t.warning, fontFamily: FONT.mono, fontSize: 6.5, fontWeight: '900' },
  progressTrack: { height: 3, borderRadius: 2, backgroundColor: t.border, marginTop: 8, overflow: 'hidden' }, progressFill: { height: 3, borderRadius: 2, backgroundColor: t.accent },
  operationSteps: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }, operationStep: { flex: 1, alignItems: 'center', gap: 3 }, operationStepDot: { width: 6, height: 6, borderRadius: 3, borderWidth: 1, borderColor: t.textFaint }, operationStepDone: { backgroundColor: t.success, borderColor: t.success }, operationStepRunning: { backgroundColor: t.accent, borderColor: t.accent }, operationStepText: { color: t.textFaint, fontFamily: FONT.mono, fontSize: 4.8, textAlign: 'center' },
  operationActions: { flexDirection: 'row', alignSelf: 'flex-end', gap: 6, marginTop: 8 },
  pauseOperation: { borderWidth: 1, borderColor: t.warning, borderRadius: 4, paddingHorizontal: 9, paddingVertical: 5 },
  pauseOperationText: { color: t.warning, fontFamily: FONT.mono, fontSize: 6.5, fontWeight: '800' },
  cancelOperation: { borderWidth: 1, borderColor: t.error, borderRadius: 4, paddingHorizontal: 9, paddingVertical: 5 }, cancelOperationText: { color: t.error, fontFamily: FONT.mono, fontSize: 6.5, fontWeight: '800' },
  emptyQueue: { borderWidth: 1, borderColor: t.border, borderRadius: 7, borderStyle: 'dashed', padding: 14, alignItems: 'center' }, emptyQueueText: { color: t.textFaint, fontFamily: FONT.mono, fontSize: 7 },
  controlGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, control: { width: '48.8%', minHeight: 108, borderWidth: 1, borderColor: t.border, borderRadius: 8, backgroundColor: 'rgba(6,12,22,.9)', padding: 11 }, controlIcon: { width: 34, height: 34, borderRadius: 17, borderWidth: 1, borderColor: t.border, alignItems: 'center', justifyContent: 'center' }, controlTitle: { color: t.text, fontFamily: FONT.display, fontSize: 9, marginTop: 9 }, controlDetail: { color: t.textDim, fontFamily: FONT.ui, fontSize: 8, marginTop: 4 },
