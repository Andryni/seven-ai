...
| File Organizer (scan/move/undo) | ✅ Real moves + JSON undo journal. Defaults to the app-private sandbox; on Android the user can select the real Downloads (or another) directory through scoped Storage Access Framework permission. No fake demo files or broad storage permission. |
...
| Document attachment | ✅ UTF-8 extraction for text/code/CSV/Markdown, local DOCX XML extraction, and native page-aware PDF analysis through Gemini inline data (10 MB PDF limit). Unsupported binaries are rejected honestly. |
...
- To bound device storage, chat histories retain at most 300 messages, with at most 100 saved sessions and 100 automation routines.
